...
def handler(event, context):
    ...
    if not file_id:
        logger.warning(
            "action=delete_file owner_id=%s status=rejected reason=missing_file_id",
            owner_id,
        )
        return {"statusCode": 400, "body": json.dumps({"error": "file_id required"})}

    logger.info(
        "action=delete_file owner_id=%s file_id=%s status=checking_ownership",
        owner_id,
        file_id,
    )
    ...
    if not item or item.get("owner_id") != owner_id:
        logger.warning(
            "action=delete_file owner_id=%s file_id=%s status=forbidden",
            owner_id,
            file_id,
        )
        return {"statusCode": 403, "body": json.dumps({"error": "forbidden"})}
    ...
    if delete_objects:
        s3.delete_objects(
            Bucket=BUCKET_NAME, Delete={"Objects": delete_objects, "Quiet": True}
        )

    logger.info(
        "action=delete_file owner_id=%s file_id=%s status=s3_deleted", owner_id, file_id
    )

    table.delete_item(Key={"owner_id": owner_id, "object_key": file_id})

    logger.info(
        "action=delete_file owner_id=%s file_id=%s status=ok", owner_id, file_id
    )
